App.Services = (function(lng, app, undefined) {
    ...
    var API_URL = "http://api.blibb.net/ipedrazas/bugalive-com";
    var PARAMETERS = {
        callback: '?' // jsonp
    };
    ...
    var add_bug = function(){
        ...
        var params = "key=bugalive-com&app_token=bugalive.com&01-title=" + lng.dom('#bug_title').val() +
             "&bug=" + lng.dom('#bug_description').val() + "&date=" + lng.dom('#bug_date').val() + "&assigment=" + lng.dom('#bug_assign').val() + "&status=active&tags=" + lng.dom('#bug_type').val();
